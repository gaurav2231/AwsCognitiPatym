...
import com.awsService.UserSignupRequest;
import com.awsService.UsersigninRequest;
...
public class AwsController {
 ...
 AWSCognitoIdentityProvider  cognitoClient;
	...
	// Signup
	@PostMapping("/signup")
	public String signup(@RequestBody UserSignupRequest userSignUp) 
	 {
		
		CognitoIdentityProviderClient client=CognitoIdentityProviderClient.builder().region(region).build();

		AttributeType attributeType = AttributeType.builder().name("email")
				                      .value(userSignUp.getEmail()).build();
		...
		try {
			...
			SignUpRequest signUp=SignUpRequest.builder().
					...
					clientId(clientId).password(userSignUp.getPassword()). secretHash(secretVal).build();
	...
	return "User Registered Successfully";
	                        ...
	                        // Signin
	@PostMapping("/signin")      
	public UserSignInResponse login(@RequestBody UsersigninRequest usersigninRequest) throws Exception{
...
try {
     ...
     challengeResponses.put("SECRET_HASH", calculateSecretHash(clientId, clientSecret,usersigninRequest.getUsername()));
